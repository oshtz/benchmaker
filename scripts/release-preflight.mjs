...
function checkDocs() {
  const runbook = readText('RELEASE_RUNBOOK.md')
  const tauriAssessment = readText('TAURI2_MIGRATION_ASSESSMENT.md')

  for (const snippet of [
    'WINDOWS_CERTIFICATE',
    'APPLE_CERTIFICATE',
    'openrouter_minimal_completion_smoke_from_stored_key',
    'Benchmaker-Portable.exe.sha256',
  ]) {
    if (runbook.includes(snippet)) {
      pass(`release runbook includes: ${snippet}`)
    } else {
      fail(`release runbook missing: ${snippet}`)
    }
  }

  if (tauriAssessment.includes('Do not migrate as part of the current hardening branch')) {
    pass('Tauri 2 assessment records defer decision')
  } else {
    fail('Tauri 2 assessment missing defer decision')
  }
}
